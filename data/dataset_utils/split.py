...
def get_split_ratio(labels, train_size=0.6, val_size=0.2, test_size=None, **kwargs):
    if kwargs['train_ratio']!= None:
        train_size = kwargs['train_ratio']
        val_size = kwargs['val_ratio']
    if test_size is None:
        test_size = 1 - train_size - val_size
    assert train_size + val_size + test_size == 1
    n_val = round(val_size * len(labels))
    indices = np.arange(len(labels))
    train_indices, val_test_indices = train_test_split(indices, train_size=train_size, stratify=labels, random_state=0)
    val_indices, test_indices = train_test_split(val_test_indices, train_size=n_val, stratify=labels[val_test_indices], random_state=0)
...
def get_split_ratio_texas(labels, train_size=0.6, val_size=0.2, test_size=None, **kwargs):
    if test_size is None:
        test_size = 1 - train_size - val_size
    assert train_size + val_size + test_size == 1
    n_val = round(val_size * len(labels))
    indices = np.arange(len(labels))
    train_indices, val_test_indices = train_test_split(indices, train_size=train_size, stratify=None, random_state=0)
    val_indices, test_indices = train_test_split(val_test_indices, train_size=n_val, stratify=labels[val_test_indices], random_state=0)
...
def get_split(labels, split_params, name=None):
    if 'train_examples_per_class' in split_params:
        return get_split_per_class(labels, **split_params)
    else:
        if name == 'texas':
            return get_split_ratio_texas(labels, **split_params)
        else:
...
if __name__ == '__main__':
    ...
    np.random.seed(0)
    dataset = pyg_load_dataset('texas')
